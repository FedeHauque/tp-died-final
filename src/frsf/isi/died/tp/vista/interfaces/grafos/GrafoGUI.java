...
public class GrafoGUI extends JPanel {
    ...
    private List<VerticeGUI> vertices;
    private List<AristaGUI> aristas;

    private AristaGUI auxiliar;
    ...
    public GrafoGUI() {
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent event) {
                VerticeGUI vDestino = clicEnUnNodo(event.getPoint());
        ...
        addMouseMotionListener(new MouseAdapter() {
            public void mouseDragged(MouseEvent event) {
                VerticeGUI vOrigen = clicEnUnNodo(event.getPoint());
                if (auxiliar==null && vOrigen != null) {
                    auxiliar = new AristaGUI();                    
    ...
    public void agregar(AristaGUI arista){
        this.aristas.add(arista);
    }    
    
    public void quitar(AristaGUI arista){
        this.aristas.remove(arista);
    }
    
    public void agregar(VerticeGUI vert){
        this.vertices.add(vert);
    }
    
    private void dibujarVertices(Graphics2D g2d) {
        for (VerticeGUI v : this.vertices) {
    ...
    private VerticeGUI clicEnUnNodo(Point p) {
        for (VerticeGUI v : this.vertices) {
